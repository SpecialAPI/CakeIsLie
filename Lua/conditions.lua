...
condlist =
{
	...
	lying = function(params,checkedconds,checkedconds_,cdata)
			local unitid,x,y = cdata.unitid,cdata.x,cdata.y
			...
			local lie = hasfeature(name, "is", "lie", unitid, x, y) ~= nil
			if(lie and #params == 2 and params[1] ~= nil and params[2] ~= nil) then
				if(hasfeatureorig(name, params[2], params[1], unitid, x, y)) then
					lie = false
				end
			end

			return lie, checkedconds
		end,
	nolying = function(params,checkedconds,checkedconds_,cdata)
			local unitid,x,y = cdata.unitid,cdata.x,cdata.y
			
			if (unitid ~= 1 and unitid ~= 2) then
				return hasfeature(getname(mmf.newObject(unitid)), "is", "lie", unitid, x, y) == nil, checkedconds
			elseif(unitid == 1) then
				return hasfeature("level", "is", "lie", 1, x, y) == nil, checkedconds
			else
				return hasfeature("empty", "is", "lie", 2, x, y) == nil, checkedconds
			end
		end,
}