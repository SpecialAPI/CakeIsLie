function findfeature(rule1,rule2,rule3)
	...
	local result = {}
	local rule = ""
	local allowlie = featureindex["lie"] ~= nil
		...
		for i,rules in ipairs(featureindex[rule1]) do
			...
			if (conds[1] ~= "never") then
				if (rule[1] == rule1) and (rule[2] == rule2) then
					local thirdthingy = rule[3]
					if(allowlie) then
						conds = copyconds({}, conds)
						if(string.sub(rule[3], 1, 4) == "not ") then
							thirdthingy = string.sub(rule[3], 5)
							conds = copyconds(conds, {{"lying", {rule[2], thirdthingy}}})
						else
							conds = copyconds(conds, {{"nolying"}})
						end
					end
					local baserule = {rule[1],rule[2],thirdthingy}
	...
	if(allowlie) then
		if (featureindex["not " .. tostring(rule3)] ~= nil) and (featureindex[rule1] == nil) then
			for i,rules in ipairs(featureindex["not " .. rule3]) do
				local rule = rules[1]
				local conds = rules[2]
				
				if (conds[1] ~= "never") then
					if (rule[3] == "not " .. rule3) and (rule[2] == rule2) then
						local baserule = {rule[1],rule[2],string.sub(rule[3], 5)}
						conds = copyconds({}, conds)
						conds = copyconds(conds, {{"lying", {rule[2], string.sub(rule[3], 5)}}})
						table.insert(options, {baserule,conds})
					end
				end
			end
...
function hasfeature(rule1,rule2,rule3,unitid,x,y,checkedconds,ignorebroken_)
	...
	local isThisLying = false
	if(allowlie) then
		isThisLying = hasfeature(rule1, "is", "lie", unitid, x, y, checkedconds, ignorebroken)
	end
	...
	if (rule1 ~= nil) and (rule2 ~= nil) and (rule3 ~= nil) then
		if (featureindex[rule1] ~= nil) then
			for i,rules in ipairs(featureindex[rule1]) do
				...
				if(isThisLying) then
					if(string.sub(rule[3], 1, 4) == "not ") then
						thirdthingy = string.sub(rule[3], 5)
						can = hasfeatureorig(rule[1], rule[2], thirdthingy, unitid, x, y) == nil
					else
						can = false
					end
				end
				if (can and conds[1] ~= "never") then
					if (rule[1] == rule1) and (rule[2] == rule2) and (thirdthingy == rule3) then
		...
		if (string.sub(rule1,1,5) == "text_") and (featureindex["text"] ~= nil) then
			for i,rules in ipairs(featureindex["text"]) do
				...
				if (can and conds[1] ~= "never") then
					if (rule[1] == "text") and (rule[2] == rule2) and (thirdthingy == rule3) then
						if testcond(conds,unitid,x,y,nil,nil,checkedconds,ignorebroken) then
							return true
						end
					end
	...
	if (rule3 ~= nil) and (rule2 ~= nil) and (rule1 ~= nil) then
		if (featureindex[rule3] ~= nil and not isThisLying) then
			for i,rules in ipairs(featureindex[rule3]) do
				local rule = rules[1]
				local conds = rules[2]
		...
		elseif(featureindex["not " .. rule3] ~= nil and isThisLying) then
			for i,rules in ipairs(featureindex["not " .. rule3]) do
				local rule = rules[1]
				local conds = rules[2]
				local thirdthingy = rule[3]
				if(string.sub(rule[3], 1, 4) == "not ") and hasfeatureorig(rule[1], rule[2], string.sub(rule[3], 5), unitid, x, y) == nil then
					thirdthingy = string.sub(rule[3], 5)
					if (conds[1] ~= "never") then
						if (rule[1] == rule1) and (rule[2] == rule2) and (thirdthingy == rule3) then
							if testcond(conds,unitid,x,y,nil,nil,checkedconds,ignorebroken) then
		...
		if (string.sub(rule3,1,5) == "text_") and (featureindex["text"] ~= nil) then
			for i,rules in ipairs(featureindex["text"]) do
				...
				if (can and conds[1] ~= "never") then
					if (rule[1] == rule1) and (rule[2] == rule2) and (thirdthingy == "text") then
						if testcond(conds,unitid,x,y,nil,nil,checkedconds,ignorebroken) then
							return true
						end
					end
...
function hasfeatureorig(rule1,rule2,rule3,unitid,x,y,checkedconds,ignorebroken_)
	local ignorebroken = false
	if (ignorebroken_ ~= nil) then
		ignorebroken = ignorebroken_
	end
	...
	if (rule1 ~= nil) and (rule2 ~= nil) and (rule3 ~= nil) then
		if (origfeatures[rule1] ~= nil) then
			for i,rules in ipairs(origfeatures[rule1]) do
				local rule = rules[1]
				local conds = rules[2]
		...
		if (string.sub(rule1,1,5) == "text_") and (origfeatures["text"] ~= nil) then
			for i,rules in ipairs(origfeatures["text"]) do
				local rule = rules[1]
				local conds = rules[2]
	...
	if (rule3 ~= nil) and (rule2 ~= nil) and (rule1 ~= nil) then
		if (origfeatures[rule3] ~= nil) then
			for i,rules in ipairs(origfeatures[rule3]) do
				local rule = rules[1]
				local conds = rules[2]
		...
		if (string.sub(rule3,1,5) == "text_") and (origfeatures["text"] ~= nil) then
			for i,rules in ipairs(origfeatures["text"]) do
				local rule = rules[1]
				local conds = rules[2]
	...
	if (origfeatures[rule2] ~= nil) and (rule1 ~= nil) and (rule3 == nil) then
		local usable = false
...
function getunitswitheffect(rule3,nolevels_,ignorethese_,checkedconds)
	...
	if (featureindex[rule3] ~= nil or (featureindex["lie"] ~= nil and featureindex["not " .. rule3] ~= nil)) then
		...
		if(featureindex["lie"] ~= nil and featureindex["not " .. rule3] ~= nil) then
			for i,v in ipairs(featureindex["not " .. rule3]) do
				local rule = v[1]
				local conds = v[2]
				
				if (rule[2] == "is") and (conds[1] ~= "never") and (findnoun(rule[1],nlist.brief) == false) then
					table.insert(group, {rule[1], conds, true})
				end
			end
		end
		...
		for i,v in ipairs(group) do
			if (v[1] ~= "empty") then
				...
				if (fgroupmembers ~= nil) and valid then
					for a,b in ipairs(fgroupmembers) do
						if(hasfeature(name, "is", "lie", b) == islie) and (islie == nil or hasfeatureorig(name, "is", rule3, b) == nil) then
...
function getunitswithverb(rule2,ignorethese_,checkedconds)
	...
	if (featureindex[rule2] ~= nil) then
		for i,v in ipairs(featureindex[rule2]) do
			...
			if (rule[2] == rule2) and (conds[1] ~= "never") and (findnoun(rule[1],nlist.brief) == false) then
				...
				if (i ~= "empty") then
					...
					if (fgroupmembers ~= nil) and (#fgroupmembers > 0) then
						for c,d in ipairs(v) do
							local thingy = d[1]
							local lying = nil
							if(string.sub(thingy, 1, 4) == "not ") then
								lying = true
								thingy = string.sub(thingy, 5)
							end

							table.insert(result, {thingy,{},name})
							local thisthisresult = result[#result][2]
							
							for a,b in ipairs(fgroupmembers) do
								if testcond(d[2],b,nil,nil,nil,nil,checkedconds) and hasfeature(name, "is", "lie", b) == lying and (lying == nil or hasfeatureorig(name, rule2, thingy, b) == nil) then
									local unit = mmf.newObject(b)
					...
					if (#empties > 0) then
						for c,d in ipairs(v) do
							...
							for e,f in ipairs(empties) do
								...
								if testcond(d[2],2,x,y,nil,nil,checkedconds) and hasfeature("empty", "is", "lie", 2, x, y) == lying and (lying == nil or hasfeatureorig("empty", rule2, thingy, 2, x, y) == nil) then
									local valid = true
					...
					if (#empties > 0) then
						for a,b in ipairs(empties) do
							...
							for c,d in ipairs(v) do
								...
								if testcond(d[2],2,x,y,nil,nil,checkedconds) and hasfeature("empty", "is", "lie", 2, x, y) == lying and (lying == nil or hasfeature("empty", rule2, thingy, 2, x, y) == nil) then
									table.insert(myverbs, thingy)
								end
							end
...
function hasfeature_count(rule1,rule2,rule3_,unitid,x,y,checkedconds)
	local result = 0
	local rule3 = rule3_
	local allowlie = featureindex["lie"] ~= nil
	local lying = false
	...
	if(allowlie and hasfeature(rule1, "is", "lie", unitid, x, y, checkedconds) and rule3 ~= nil) then
		rule3 = "not " .. rule3
		lying = true
	end
	...
	if (featureindex[rule1] ~= nil) and (featureindex[rule3] ~= nil or (allowlie and featureindex["not " .. rule3] ~= nil)) and (rule2 ~= nil) then
		if (#featureindex[rule1] < #featureindex[rule3]) then
			for i,v in ipairs(featureindex[rule1]) do
				...
				if (rule[1] == rule1) and (rule[2] == rule2) and (rule[3] == rule3) then
					if testcond(conds,unitid,x,y,nil,nil,checkedconds) and (not lying or hasfeatureorig(rule[1], rule[2], rule[3], unitid) == nil) then
						result = result + 1
					end
				end
			end
