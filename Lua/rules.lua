...
function addoption(option,conds_,ids,visible,notrule,tags_)
	...
	if (#option == 3) then
		...
		if (origfeatures[effect] == nil) then
			origfeatures[effect] = {}
		end
		
		if (origfeatures[target] == nil) then
			origfeatures[target] = {}
		end
		
		if (origfeatures[verb] == nil) then
			origfeatures[verb] = {}
		end
		
		table.insert(featureindex[effect], rule)
		table.insert(featureindex[verb], rule)
		table.insert(origfeatures[effect], origrule)
		table.insert(origfeatures[verb], origrule)
		
		if (target ~= effect) then
			table.insert(featureindex[target], rule)
			table.insert(origfeatures[target], origrule)
		end
