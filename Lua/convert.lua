function conversion(dolevels_)
	...
	for i,v in pairs(features) do
		...
		if (operator == "is") or (operator == "write") or (operator == "become") then
			...
			if (getmat(thing) ~= nil) or (allowlie and string.sub(thing, 1, 4) == "not " and getmat(string.sub(thing, 5)) ~= nil) or (thing == "not " .. name) or (thing == "all") or (unitreference[thing] ~= nil) or (allowlie and string.sub(thing, 1, 4) == "not " and unitreference[string.sub(thing, 5)] ~= nil) or ((thing == "text") and (unitreference["text_text"] ~= nil)) or (thing == "revert") or ((operator == "write") and getmat_text("text_" .. name)) then
				if (featureindex[name] ~= nil) and (alreadydone[name] == nil) then
					...
					for a,b in ipairs(featureindex[name]) do
						...
						if(allowlie) then
							if(string.sub(object, 1, 4) == "not ") then
								object = string.sub(object, 5)
							else
								object = "not " .. object
							end
							if (verb == "is") or (verb == "become") then
								if (target == name) and (object ~= "word") and ((object ~= name) or (verb == "become")) then
									if (object ~= "text") and (object ~= "revert") then
										if (object == "not " .. name) then
											table.insert(output, {"error", conds, "is", true, {target, verb, object}})
										else
											for d,mat in pairs(objectlist) do
												if (string.sub(d, 1, 5) ~= "group") and (d == object) then
													table.insert(output, {object, conds, "is", true, {target, verb, object}})
												end
											end
										end
									elseif (name ~= object) or (verb == "become") then
										if (object ~= "revert") then
											...
											table.insert(output, 1, {object, conds, "is", true, {target, verb, object}})
										end
									end
								end
							elseif (verb == "write") then
								if (string.sub(object, 1, 4) ~= "not ") and (target == name) then
									table.insert(output, {object, conds, "write", true, {target, verb, object}})
								end
							end
						end
					end
					...
					for k,v3 in pairs(output) do
						...
						if (op == "is") then
							if (findnoun(object,nlist.brief) == false) and (object ~= "word") and (object ~= "text") then
								table.insert(conversions, {object, conds, op, reqlie, ogrule})
							elseif (object == "all") then
								--[[
								addaction(0,{"createall",{name,conds},dolevels})
								createall({name,conds})
								]]--
								table.insert(conversions, {"createall",conds, op, reqlie, ogrule})
							elseif (object == "text") then
								table.insert(conversions, {"text_" .. name, conds, op, reqlie, ogrule})
							end
				...
				for i,unitid in pairs(targets) do
					...
					if (unit.flags[CONVERTED] == false) then
						for a,matdata in pairs(mats) do
							...
							local op = matdata[3]
							local reqlie = matdata[4]
							local ogrule = matdata[5]
							...
							if (reverting == false) then
								...
								if testcond(conds,unit.fixed) and objectfound and reqlie == hasfeature(name, "is", "lie", unitid, x, y) and (reqlie == nil or ogrule == nil or hasfeatureorig(ogrule[1], ogrule[2], ogrule[3], unitid, x, y) == nil) and (reqlie == nil or hasfeatureorig(name, "is", "not " .. name, unitid, x, y) == nil) then
									local ingameid = 0
					...
					if empty then
						for a,matdata in pairs(mats) do
							...
							if (mat2 ~= "empty") and objectfound then
								if testcond(conds,2,i,j) and reqlie == hasfeature("empty", "is", "lie", 2, i, j) and (reqlie == nil or ogrule == nil or hasfeatureorig(ogrule[1], ogrule[2], ogrule[3], 2, i, j) == nil) and (reqlie == nil or hasfeatureorig("empty", "is", "not empty", 2, i, j) == nil) then
